            ...
            <template v-for="(item,index) in navMenuList">
                <el-menu-item v-if="index<showCount" :index="item.id+''"
                    ...
                    <template slot="title">
                        ...
                        <!-- <svg-icon :icon-class="item.meta.icon ? item.meta.icon : ''" /> -->
                        <!-- <span>{{generateTitle(item)}}</span> -->
                        <item v-if="item.meta" :icon="item.meta && item.meta.icon" :title="item.meta.title" />
                        <!-- <item v-else :icon="item.children[0].meta && item.children[0].meta.icon" :title="item.children[0].meta.title" /> -->
                    </template>
                </el-menu-item>
            </template>
                ...
                this.$store.dispatch("GetMenu", item.id).then(data => {
                    ...
                    this.$router.push({
                        path: this.$router.$avueRouter.getPath({
                            ...
                            src: itemActive.path,
                            i18n: (itemActive.meta.title || {}).i18n
                        })
                    });
                });
            }
