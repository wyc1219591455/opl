...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            broadcast(fileList) {
                fileList.length ? fileList[0].name = baseUrl + "avatar" + fileList[0].name: [];
                this.dataFileList = fileList;
                let urlList = fileList.map(item=>{
                    return item.response && item.response.data.filePath ? item.response.data.filePath : '';
                });
                if (this.limit==1) {
                    if (this.listType === 'paper') {
