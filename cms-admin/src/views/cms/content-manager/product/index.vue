<template>
  <div class="app-container">
    <basic-container>
      ...
      <addPage path="/productAdd"></addPage>
      <!--表格部分-->
      <vxe-table
          highlight-hover-row
          resizable
        ...
        <vxe-table-column field="desc" title="分类描述" show-overflow min-width="100"></vxe-table-column>
        <vxe-table-column field="metaDescription" title="SEO描述" show-overflow min-width="100"></vxe-table-column>
        <vxe-table-column field="metaKeywords" show-overflow title="SEO关键字"></vxe-table-column>
        <vxe-table-column field="language" title="语言" width="100">
          <template v-slot:default="{ row }">{{ languageText[row.language] }}</template>
        </vxe-table-column>
        <vxe-table-column field="sort" title="排序" width="50"></vxe-table-column>
...
export default {
  ...
  methods: {
    ...
    getArticleCategoryList() {
      ...
      api.queryProductList(params, 'get').then((res) => {
