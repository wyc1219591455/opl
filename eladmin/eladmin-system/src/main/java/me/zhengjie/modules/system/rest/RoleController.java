...
public class RoleController {
   ...
   /* @ApiOperation("返回全部的角色")
    ...
    @GetMapping(value = "/all")
    @PreAuthorize("@el.check('roles:list','user:add','user:edit')")
    public Object query(@PageableDefault(value = 2000, sort = {"level"}, direction = Sort.Direction.ASC) Pageable pageable){
    ...
    @GetMapping
    @PreAuthorize("@el.check('roles:list')")
    public Object query(RoleQueryCriteria criteria, Pageable pageable){
   ...
   /* @ApiOperation("获取用户级别")
    ...
    public RoleDto create(@Validated @RequestBody Role resources){
        if (resources.getId() != null) {
            throw new BadRequestException("新增角色已拥有ID");
        }
        getLevels(resources.getLevel());
    ...
    @PutMapping
    @PreAuthorize("@el.check('roles:edit')")
    public void update(@Validated(Role.Update.class) @RequestBody Role resources){
        getLevels(resources.getLevel());
        roleService.update(resources);
    }
