...
@Api(tags = "系统：部门管理")
@RequestMapping(value="/api/dept",produces = "application/json;charset=utf8")
public class DeptController {
   ...
   /* @Log("查询部门")
    ...
    @GetMapping
    @PreAuthorize("@el.check('user:list','dept:list')")
    public Object query(DeptQueryCriteria criteria) throws Exception {
        List<DeptDto> deptDtos = deptService.queryAll(criteria, true);
  ...
  /*  @Log("查询部门")
    ...
    @PostMapping
    @PreAuthorize("@el.check('dept:add')")
    public DeptDto create(@Validated @RequestBody Dept resources){
        if (resources.getId() != null) {
            throw new BadRequestException("A new "+ ENTITY_NAME +" cannot already have an ID");
        }
        return deptService.create(resources);
    }
    ...
    @PutMapping
    @PreAuthorize("@el.check('dept:edit')")
    public void update(@Validated(Dept.Update.class) @RequestBody Dept resources){
        deptService.update(resources);
    }
